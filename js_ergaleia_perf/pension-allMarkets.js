...
const yearSelected = document.getElementById("yearStartInvest");
const anaprosarmogiSelected = document.getElementById("anaprosarmogi");
yearSelected.value = startYear;
amount_selected.value = startAmount;
...
const changeValuesMarket = () => {
    console.log(Number($("#anualadjustment").val()));
    if (Number($('#neededinitialmonthlyinvestment').val())) {
        amount_selected.value = Math.trunc(Number($('#neededinitialmonthlyinvestment').val()) * 12);
    } else {
        amount_selected.value = startAmount;
    }
    if (Number($("#anualadjustment").val())) {
        anaprosarmogiSelected.value = Number($("#anualadjustment").val());
    } else {
        anaprosarmogiSelected.value = startAnaprosarmogi;
    }
    yearSelected.value = startYear;
}
const ypologiseClicked = document.getElementById('ypologise').addEventListener(
    'click', () => {
        const cards_of_markets = document.querySelectorAll(".card-title");
        cards_of_markets
            .forEach((card) => {cal(card)});
...
function cal(card) {
    const belongs = card.textContent;
    const parent = card.parentElement.children[1];
    for (const market in markets) {
        if (markets[market].name === belongs) { // only if specific card can take market info
            let yearClientSelected = Number(yearSelected.value);
            // check if right year is given
            if (!yearClientSelected || yearClientSelected < 1998 || yearClientSelected > 2020) {
                yearClientSelected = startYear;
                yearSelected.value = startYear;
            }
            let amount_every_year = Number(amount_selected.value);
            // check for right year to given
            if (!amount_every_year || amount_every_year < 0) {
                amount_every_year = startAmount;
                amount_selected.value = startAmount;
            }
            let index;
            const formatInTwoDigit = (number) => {
                return number.toLocaleString('en-US', {
                    minimumIntegerDigits: 2,
                    useGrouping: false
                });
            };
            // find which index to take as start
            markets[market].yearsLabels.forEach(
                (element, index1) => {
                    const year1 = element.slice(4);
                    if (year1 === formatInTwoDigit(yearClientSelected % 100).toString()) {
                        index = index1;
                    }
                }
            );
            let anaprosarmogi = Number(anaprosarmogiSelected.value);
            // console.log(anaprosarmogi);
            if (!anaprosarmogi) {
                anaprosarmogi = 0;
                anaprosarmogiSelected.value = '0';
            }
            // console.log(anaprosarmogi, "ANAPROSARMOGI");
            const amountPerYear = [];
            console.log(markets[market].performances.length, index);
            for (let counter = index; counter < markets[market].performances.length; counter++) {
                amountPerYear.push(amount_every_year);
                amount_every_year = amount_every_year * (1 + anaprosarmogi / 100);
            }
            // console.log(amountPerYear);
            let sum_ = 0;
            for (let counter = index; counter < markets[market].performances.length; counter++) {
                // console.log(sum_);
                sum_ = (sum_ * ( 1 + markets[market].performances[counter] / 100)) + amountPerYear[counter - index];
            }
            // only one year investment
            ...
            //     });
            const num_nic_format = numberWithCommas(Math.trunc(sum_));
            const string = `<p class="card-text">Σύνολο: <b>${num_nic_format}€</b> <i>Χρόνια: ${markets[market].performances.length - index}</i></p>`;
            $(parent).empty();
...
$(document).ready(function () {
    $("#apodoshxrhmatwn,#mone,#age,#averageIncome,#workyearsuntilnow,#workyearsuntilpension,#syntelanapl,#initpension,#nationalpension,#totalpension,#wantedpension,#neededprivatepension,#neededcapitaluntil85,#neededinitialmonthlyinvestment,#initialmonthlyinvestment,#initialcapital,#anualperformance,#anualadjustment,#endingvalue,#endingvaluepermonth")
        .keyup(function () {
            ...
            $("#apodoshxrhmatwn").val("+ "+parseFloat(calculate_performance(Number($("#workyearsuntilpension").val()),
                Number($("#initialmonthlyinvestment").val()),
                Number($("#initialcapital").val()),
                Number($("#endingvalue").val())
            )).toFixed(2)+ " %");
            ...
            getMarketsInfo();
            if (annual_perfomance >= 3 && annual_perfomance <= 22) {
                document.getElementById("neededinitialmonthlyinvestment")
                    .classList.add('super');
                //TODO
...
function calculate_performance(investment_years,initial_monthly_investment,initial_capital,ending_value){
    let money_saved = investment_years*initial_monthly_investment + initial_capital;
    return 100 * ((ending_value - money_saved) / money_saved )
}
...
function ending_total_value_after_revenue_input(starting_monthly_capital, starting_capital, year_revenue, year_change, years_until_pension) {
    ...
    for (i = 0; i < years_until_pension; i++) {
        ...
        starting_monthly_capital = starting_monthly_capital + (starting_monthly_capital * (year_change / 100));
        total_value = total_value + (starting_monthly_capital);
        total_value = total_value + (total_value * (year_revenue / 100));
    }
    return total_value;
}
